...
contract DavinciDaoMultiCollectionTest is Test {
    ...
    function testMultiCollectionMultiDelegate() public {
        ...
        // Delegate tokens from first collection to bob (bob has weight 0)
    ...
    function testUpdateDelegationMultiCollection() public {
        ...
        fromProofs[0] = DavinciDao.ProofInput({account: bob, currentWeight: 2, siblings: new uint256[](1)});
        fromProofs[0].siblings[0] = 0; // Simplified for test

        census.updateDelegation(charlie, 0, collection1Ids, 0, fromProofs, emptyProof);

        // After update: Bob weight=1, Charlie weight=1
        // Weight verification is done via subgraph or WeightChanged events
